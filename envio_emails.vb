...
Public Class envio_emails
    ...
    Public Shared Sub enviarEmailsTrial()
        ...
        Try
            ...
            clientes = clientesDao.obterclientesTrial()
            Dim I As Integer
            For I = 0 To clientes.Count

                Try
                    Dim dtHj = Date.Today
                    Dim dtInc = clientes(I).data_inc
                    Dim pendencia As String
                    Dim template As New templateVO

                    If (DateDiff(DateInterval.Day, dtInc.Date, dtHj) = 20) Then

                        template = Funcs.retornaTemplate("4")
                        enviarEmail(clientes(I), template)


                    ElseIf ((DateDiff(DateInterval.Day, dtInc.Date, dtHj) = 15) Or
                            (DateDiff(DateInterval.Day, dtInc.Date, dtHj) = 10) Or
                            (DateDiff(DateInterval.Day, dtInc.Date, dtHj) = 5)) Then
                        pendencia = Funcs.verificaPendencia(clientes(I))
                        template = Funcs.retornaTemplate(pendencia)
    ...
    Private Shared Sub enviarEmail(ByVal cliente As clientesVO, ByVal template As templateVO)
        ...
        corpo = corpo.Replace("[CONTATO]", cliente.nomeUser)
        corpo = corpo.Replace("@nomeEmpresa", cliente.nomeEmp)
        'par1 = idLogin / par2 = idEmpresa / par3 = idRelLogin
        corpo = corpo.Replace("[link_remover]", "http://srv1.nfecommerce.com.br/site/optout.php?par1=" + CStr(cliente.idLogin) +
                              "&par2=" + CStr(cliente.idEmpresa) + "&par3=" + CStr(cliente.idRelLogin) + "&token=" + cliente.token)
